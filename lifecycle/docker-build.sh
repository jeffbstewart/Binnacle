#!/bin/bash
# Build the Binnacle Docker image, push it to the private registry on the
# NAS, and ask Watchtower to pull + restart the running container.
#
# Configuration lives in secrets/deploy.agent_visible_env (gitignored).
# Copy secrets/example.deploy.env to that path on first use.
...
echo "=== Building binnacle (${TIMESTAMP}) ==="
...
if [ -n "${WATCHTOWER_TOKEN:-}" ]; then
    ...
    echo "Deploy manually: docker-compose pull && docker-compose up -d"
fi
